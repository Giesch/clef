...
- [ ] make sure this still works on windows
  where does window_handle_hack go?
- [ ] set up lint rules / forbid/deny unsafe code by crate
- [ ] move crawler usage of symphonia into an audio crate module

- [ ] look into parking_lot again
  - [ ] use parking_lot::Mutex instead iced or std?
  - [ ] use parking_lot::Once instead of once_cell?
    could also be possible to use just the stabilized portion of once_cell

- [ ] implement true gapless playback
...
- [X] listen to media keys while backgrounded
...
- [X] see if / how much of the optimistic time stuff can be removed app-side
