...
- [X] unit tests for update
  - [X] make an inner type for the non-thready ui state
  - [X] only pass the inner type to inner update
  - [X] test the limited inner update
...
- [ ] add a hash or id to resized image names
  two missing album titles results in duplicated names
