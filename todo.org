...
- [ ] gapless playback
  ...
  - [ ] try the preloader version and see if it works out
    - [ ] launch a preloader thread in a ready state when spawning the player
    - [ ] send a message to the preloader in play_queue, forward, and back
      - check that it's working with a log message or something
  - [ ] build a decoder on the preloader, and read 10 seconds or the whole file
    could also limit attempted time by the length of the current/previous track
     don't include any audio device anything
    - [ ] first, load a decoder without any decoding done at first
    - [ ] send the prepared decoder to the player thread
