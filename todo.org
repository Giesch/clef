...
- [-] fix media controls while app is backgrounded
  ...
  - [X] then, figure out how to handle moving the logic over
    the optimistic update on back will be weird
      ...
      seems fine
  - [ ] will publishing metadata also have to happen off-ui-thread for the same reason?
      like would they just not be updated if the problem is that ui update isn't running
      yeah, it's delayed until the app is foregrounded again
  - [ ] does the window requirement on windows mean that it has to be on ui thread?
      how does that work? does update actually get called when backgrounded on windows?
      do we just have to get the window handle earlier somehow?
